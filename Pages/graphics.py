...
def CreateWordCloud(depressed_Terms, ImageName):
    ...
    # Preprocess the text
    depressed_wordcloud_text = Preprocessing.remove_repeated_words(
        depressed_wordcloud_text
    )
...
def chart():
    ...
    fig = go.Figure(
        data=[go.Bar(x=analysis_data, y=months, marker=dict(color=bar_color))]
    )
    ...
    fig.update_layout(
        ...
        xaxis=dict(
            tickfont=dict(size=25),  # Font size for x-axis tick labels
            tickangle=45,  # Rotate tick labels for better visibility
            showgrid=False,  # Hide gridlines
            zeroline=False,  # Hide zero line
        ),
        yaxis=dict(tickfont=dict(size=25)),  # Font size for y-axis tick labels
